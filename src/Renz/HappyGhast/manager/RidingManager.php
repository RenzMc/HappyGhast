...
class RidingManager implements Listener {
        ...
        private function dismount(Player $player): void {
        ...
        public function onPlayerToggleSneak(PlayerToggleSneakEvent $event): void {
                $player = $event->getPlayer();
                if ($event->isSneaking() && $this->isRiding($player)) {
                        $this->dismount($player);
                }
        }
        ...
        public function onPlayerMove(PlayerMoveEvent $event): void {
                ...
                if ($isDriver) {
                        ...
                        $speed = 0.5;
                        $entity->setMotion($directionVector->multiply($speed));
                }
        ...
        public function onPlayerJump(PlayerJumpEvent $event): void {
                ...
                $passengers = $entity->getPassengerIds();
                if (isset($passengers[0]) && $passengers[0] === $player->getId()) {
                        $entity->setMotion($entity->getMotion()->addVector(0, 0.3, 0));
                }
        }
